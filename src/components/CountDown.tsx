...
const getDate = () => {
  ...
  const minutes = endDate.subtract(days, 'days').subtract(hours, 'hours').diff(date, 'minutes');
...
const CountDown = () => {
  ...
  return (
    <h2 class="countdown">
      {`${currentDate().days} DNI ${currentDate().hours} GODZIN ${currentDate().minutes} MINUT`}
    </h2>
  );
};
